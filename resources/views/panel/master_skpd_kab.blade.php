            ...
            <div class="hpanel @if (!Session::has('showCreate')) collapsed @endif">
                <div class="panel-heading">
                    <div class="panel-tools">
                        <button class="btn btn-xs btn-info showhide">Tambah SKPD <i class="fa fa-chevron-up"></i></button>
                    </div>
                    &nbsp;
                </div>
                <div class="panel-body">
                    <form method="POST" id="FormInsert" class="form-horizontal">
                        @csrf
                        <div class="form-group"><label class="col-sm-2 control-label">Kode SKPD</label>
                            <div class="col-sm-10">
                                <input type="text" name="KodeSkpd" class="form-control" required>
                            </div>
                        </div>
                        <div class="form-group"><label class="col-sm-2 control-label">Nama SKPD</label>
                            <div class="col-sm-10">
                                <input type="text" name="NamaSkpd" class="form-control" required>
                            </div>
                        </div>
                    ...
                    <form method="POST" id="FormEdit" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label">Kode</label>
                                <div class="col-sm-10">
                                    <input type="text" id="KodeSkpdEdit" name="KodeSkpd" class="form-control" required>
                                </div>
                            </div>
                            <div class="form-group"><label class="col-sm-2 control-label">Nama</label>
                                <div class="col-sm-10">
                                    <input type="text" id="NamaSkpdEdit" name="NamaSkpd" class="form-control" required>
                                </div>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    var table2 = $('#datatable-slide').DataTable({
            ...
            "serverSide": true,
            "ajax": "/panel/master/skpd/kab/json",
            "columns": [
...
function AjaxSimpanData() { 
    var data = $('#FormInsert').serialize();
        $.ajax({
            type: 'POST',
            url: '/panel/master/skpd/kab/insert',
            data: data,
            success: function() {
                alertSukses();
...
function AjaxEditData() { 
    var data = $('#FormEdit').serialize();
        $.ajax({
            type: 'POST',
            url: '/panel/master/skpd/kab/edit',
            data: data,
            success: function() {
                alertSukses();
                $('#datatable-slide').DataTable().ajax.reload();
...
function AjaxHapusData(id) { 
    var data = id;
        $.ajax({
            type: 'GET',
            url: '/panel/master/skpd/kab/delete/'+id,
            data: data,
            success: function() {
                alertSukses();
                $('#datatable-slide').DataTable().ajax.reload();
...
function EditData(id, kode, nama) { 
    ...
    document.getElementById("KodeSkpdEdit").value= kode;
    document.getElementById("NamaSkpdEdit").value= nama;
}
