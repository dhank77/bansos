                ...
                <div class="modal-content">
                    <form method="POST" id="FormEdit" class="form-horizontal" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group"><label class="col-sm-2 control-label">Kode</label>
                                <div class="col-sm-10">
                                    <input type="text" id="KodeEdit" name="Kode" class="form-control" required>
                                </div>
                            </div>
                            <div class="form-group"><label class="col-sm-2 control-label">Nama</label>
                                <div class="col-sm-10">
...
$(document).ready(function() {
    ...
    var table2 = $('#datatable-slide').DataTable({
            ...
            "serverSide": true,
            "ajax": "/panel/master/data/ms_program/json",
            "columns": [
...
function AjaxSimpanData() { 
    var data = $('#FormInsert').serialize();
        $.ajax({
            type: 'POST',
            url: '/panel/master/ms_program/insert',
            data: data,
            success: function() {
                alertSukses();
...
function AjaxEditData() { 
    var data = $('#FormEdit').serialize();
        $.ajax({
            type: 'POST',
            url: '/panel/master/ms_program/edit',
            data: data,
            success: function() {
                alertSukses();
                $('#datatable-slide').DataTable().ajax.reload();
...
function AjaxHapusData(id) { 
    var data = id;
        $.ajax({
            type: 'GET',
            url: '/panel/master/ms_program/delete/'+id,
            data: data,
            success: function() {
                alertSukses();
                $('#datatable-slide').DataTable().ajax.reload();
