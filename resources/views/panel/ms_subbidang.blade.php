            ...
            <div class="hpanel @if (!Session::has('showCreate')) collapsed @endif">
                ...
                <div class="panel-body">
                    <form method="POST" id="FormInsert" class="form-horizontal">
                        ...
                        <div class="form-group"><label class="col-sm-2 control-label">Nama Sub Bidang</label>
                            <div class="col-sm-10">
...
$(document).ready(function() {
    ...
    var table2 = $('#datatable-slide').DataTable({
            ...
            "serverSide": true,
            "ajax": "/panel/master/data/ms_subbidang/json",
            "columns": [
...
function AjaxSimpanData() { 
    var data = $('#FormInsert').serialize();
        $.ajax({
            type: 'POST',
            url: '/panel/master/ms_subbidang/insert',
            data: data,
            success: function() {
                alertSukses();
...
function AjaxEditData() { 
    var data = $('#FormEdit').serialize();
        $.ajax({
            type: 'POST',
            url: '/panel/master/ms_subbidang/edit',
            data: data,
            success: function() {
                alertSukses();
                $('#datatable-slide').DataTable().ajax.reload();
...
function AjaxHapusData(id) { 
    var data = id;
        $.ajax({
            type: 'GET',
            url: '/panel/master/ms_subbidang/delete/'+id,
            data: data,
            success: function() {
                alertSukses();
                $('#datatable-slide').DataTable().ajax.reload();
...
function EditData(id, nama, kode) { 
